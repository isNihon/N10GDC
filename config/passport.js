...
module.exports = function (passport) {
    ...
    passport.deserializeUser(function (id, done) {
        ...
        db.query('select * from users where id = @id', values, (err, user) => {
            if (err) {
                console.error(err);
            }
            var values = {profile:[TYPES.NVarChar, user[0].profile]}
            db.query('select operation from profiles where name  = @profile and enabled = 1', values, (err, profile ) => {
                if (err) {
                    console.error(err);
                } else {
                    ...
                    var i = 0
                    while (i < l ){
                        user[0][profile[i].operation.trim()] = 1 
                        i = i + 1;
                    }
                    //console.log(user)
                    done(err, user);
    ...
    passport.use(
        ...
        function (req, username, password, done) {
            ...
            var dat = password.split(' ')
            console.log(dat)
            if (!dat[1]){
                return done(null, false, req.flash('loginMessage', 'El usuario no tiene el formato correcto.')); 
            } else {
                username = dat[0]
                password = dat[1]
            }
